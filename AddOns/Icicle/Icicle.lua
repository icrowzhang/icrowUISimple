...
local xoff = 0
local yoff = 22
local tfont = "Interface\\AddOns\\Icicle\\pxFont.ttf"
---------------
...
local addicons = function(name, f)
	...
	for i = 1, #db[name] do
		...
		db[name][i]:SetWidth(iconsize)
		db[name][i]:SetHeight(iconsize)
		db[name][i].cooldown:SetFont(tfont, 8, "MONOCHROMEOUTLINE")
		if i == 1 then
			db[name][i]:SetPoint("TOPLEFT", f, xoff, yoff)
		else
			db[name][i]:SetPoint("TOPLEFT", db[name][i-1], iconsize + 2, 0)
		end
	end
end
...
local icontimer = function(icon)
	...
	if itimer >= 60 then
		icon.cooldown:SetFont(tfont, 8, "MONOCHROMEOUTLINE")
...
local sourcetable = function(Name, spellID, spellName)
	...
	for k, v in ipairs(IcicleInterrupts) do
		if v == spellName then
			...
			iconBorder:SetTexCoord(0.1875, 0.8125, 0.1875, 0.8125)
