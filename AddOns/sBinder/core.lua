...
SpellBinder.makeSpellsList = function(self, scroll, delete)
   ...
   for i, spell in ipairs(DB.spells) do
      ...
      if v then
         ...
         bf.delete:GetNormalTexture():SetVertexColor(.8, 0, 0)
         ...
         bf:SetScript("OnEnter", function(self) bf.delete:GetNormalTexture():SetVertexColor(1, 0, 0) self:SetBackdrop({ bgFile = 'Interface\\Buttons\\WHITE8x8' }) self:SetBackdropColor(.2, .2, .2, .7) end)
         bf:SetScript("OnLeave", function(self) bf.delete:GetNormalTexture():SetVertexColor(.8, 0, 0) self:SetBackdrop(nil) end)
...
SpellBinder.OpenButton:SetScript("OnShow", function(self)
   ...
   self:SetScript("OnEnter", function(self) GameTooltip:ClearLines() GameTooltip:SetOwner(self, ANCHOR_TOPLEFT) GameTooltip:AddLine('SpellBinder') GameTooltip:Show() end)
   self:SetScript("OnLeave", function() GameTooltip:Hide() end)
end)

SpellBinder.OpenButton:SetScript("OnClick", function(self) 
   if InCombatLockdown() then SpellBinder:Hide() return end
   if SpellBinder:IsVisible() 
      then SpellBinder:Hide()
      SpellBinder.sbOpen = false
   else 
      SpellBinder:Show()
...
SpellBinder:SetScript("OnEvent", function(self, event, ...)
   if event == "PLAYER_LOGIN" then
      SpellBinderDB = _G.SpellBinderDB or {}
      SpellBinderDB[UnitName("player")] = _G.SpellBinderDB[UnitName("player")] or {}
